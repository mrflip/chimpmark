...
* For the reduce-bound cluster, let the memory constraints govern the number of tasks.

* If you have very short tasks -- a few seconds, short enough that job setup time is significant -- run slightly more mappers than you would otherwise. (My un-measured guess is one extra mapper + another per 8 cores). Also, set @mapred.job.reuse.jvm.num.tasks := -1@.

* If you are running in a streaming context, you might run either fewer or more tasks per node! Basically, start with the defaults and follow the tuning instructions below.
