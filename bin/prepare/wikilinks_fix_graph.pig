...
%default PAGELINKS_TOFIX_FILE '/tmp/wikidump/pagelinks_tofix'
...
-- 10292620	0	1984
--
PagelinksTofix = LOAD '$PAGELINKS_TOFIX_FILE' AS (src_id:long, dest_ns:int, dest_title:chararray);

--
-- JOIN to convert titles to ids
--
Pagelinks_0 = JOIN PagelinksTofix BY (dest_ns, dest_title) LEFT OUTER, PageInfoIds BY (ns, title) PARALLEL 10 ;
Pagelinks_1 = FOREACH Pagelinks_0 GENERATE PagelinksTofix::src_id AS src_id, PageInfoIds::page_id AS dest_id;
Pagelinks   = ORDER Pagelinks_1 BY src_id ASC;

-- Badlinks = FILTER Pagelinks_0 BY PageInfoIds::page_id == Null ;
-- rmf                   /tmp/badlinks 
-- STORE Badlinks INTO '/tmp/badlinks';
